...
SEASON_EPISODE_PATTERNS = [
    (re.compile(r'(?:S|Season)\s*-?\s*(\d+)', re.IGNORECASE), ('season', None)),
    (re.compile(r'Season\s*(\d+)\s*\b(?:Episode|Ep|E)\s*\d+', re.IGNORECASE), ('season', None)),
    (re.compile(r'S(?P<season>\d+)(?:E|EP)\d+', re.IGNORECASE), ('season', None)),
    (re.compile(r'S(?P<season>\d+)\s*-\s*E\d+', re.IGNORECASE), ('season', None)),

    # --- Standard SxxExx formats ---
    (re.compile(r'S(\d+)(?:E|EP)(\d+)'), ('season', 'episode')),
    (re.compile(r'S(\d+)[\s-]*(?:E|EP)(\d+)'), ('season', 'episode')),

    # --- Full text (Season X Episode Y) ---
    (re.compile(r'Season\s*(\d+)\s*Episode\s*(\d+)', re.IGNORECASE), ('season', 'episode')),

    # --- Bracketed formats [S01][E02] ---
    (re.compile(r'\[S(\d+)\]\[E(\d+)\]'), ('season', 'episode')),

    # --- Loose fallback: S01 13, S01-E02, etc. ---
    (re.compile(r'S(\d+)[^\d]*(\d+)'), ('season', 'episode')),

    # --- Episode-only formats: E05, Episode 7 ---
    (re.compile(r'(?:E|EP|Episode)\s*(\d+)', re.IGNORECASE), (None, 'episode')),

    # --- Final fallback: any standalone number (guess it's an episode) ---
...
LANGUAGE_PATTERNS = [
    (re.compile(r'\b(eng|english)\b', re.IGNORECASE), "ENG"),
    (re.compile(r'\b(jpn|japanese)\b', re.IGNORECASE), "JPN"),
    (re.compile(r'\b(tel|telugu)\b', re.IGNORECASE), "TEL"),
    (re.compile(r'\b(tam|tamil)\b', re.IGNORECASE), "TAM"),
    (re.compile(r'\b(hin|hindi)\b', re.IGNORECASE), "HIN"),
    (re.compile(r'\b(sub|dub|dual|multi)\b', re.IGNORECASE), lambda m: m.group(0).upper()),
]

CLEANING_PATTERNS = [
    re.compile(r'@[\w\-]+'),  # Uploader tags (@username)
    re.compile(r'\[.*?\]'),    # Anything in brackets
    re.compile(r'\(.*?\)'),    # Anything in parentheses
    re.compile(r'[^\w\s\-\.\']'),  # Special chars except spaces, hyphens, dots, and apostrophes
    re.compile(r'[\-_]{2,}'),  # Multiple underscores/hyphens
...
def extract_language(filename: str) -> Optional[str]:
    ...
    return " ".join(languages) if languages else N
def extract_metadata(filename):
    """Extract metadata from filename with improved pattern matching"""
    # First extract the base name without extension
    ...
    # Extract season and episode with more comprehensive patterns
    season_episode_patterns = [
        (r'\[S(\d{1,2})[-_]?E(\d{1,3})\]', ('season', 'episode')),
        (r'S(\d{1,2})\s*E(\d{1,3})', ('season', 'episode')),
        (r'(\d{1,2})x(\d{1,3})', ('season', 'episode')),
        (r'Season\s*(\d{1,2})\s*Episode\s*(\d{1,3})', ('season', 'episode')),
        (r'EP?\s*(\d{1,3})', (None, 'episode')),
        (r'\[(\d{1,2})[-_](\d{1,3})\]', ('season', 'episode')),
    ]
    ...
    for pattern, (season_group, episode_group) in season_episode_patterns:
        ...
        if match:
            ...
            metadata['episode'] = match.group(2 if season_group else 1).zfill(2)
            break
    
    # Extract quality information (keep this in brackets in final output)
    quality_patterns = [
    ...
    for pattern, replacement in quality_patterns:
        match = re.search(pattern, base_name, re.IGNORECASE)
        if match:
            metadata['quality'] = replacement
            break
    
    # Extract audio/language information (keep this in brackets in final output)
    language_patterns = [
    ...
    # Extract group tag if present (like [AE])
    group_match = re.search(r'\[([A-Z]{2,})\]', base_name)
    if group_match:
        metadata['group'] = group_match.group(1)
    
    # Extract clean title by removing all metadata patterns
    title = base_name
...
def clean_title(title: str) -> str:
    ...
    # Remove uploader tags, brackets, and special characters
    for pattern in CLEANING_PATTERNS:
        title = pattern.sub(' ', title)
    
    # Remove season/episode/quality/language patterns
    ...
    # Remove season number from title if present (e.g., "Season 2")
...
def clean_filename(filename):
    """Clean filename by removing duplicate extensions and unwanted patterns"""
    if not filename:
        return ""
        
    # First extract the real extension
    base, real_ext = os.path.splitext(filename)
    
    # Remove any fake extensions in the middle
    base = re.sub(r'\.\w+$', '', base)  # Remove any extension-like patterns at end
    base = re.sub(r'\.\w+(?=\s*[\[\(-])', '', base)  # Remove extensions before brackets/hyphens
    
    # Now clean the title portion
    cleaned = clean_title(base)
    
    # Re-add the actual extension
    return f"{cleaned}{real_ext}"

def apply_template(template, metadata, ext=".mkv"):
    """Apply the template with all extensions removed from metadata fields"""
    # Clean all metadata fields to remove extensions
    for key in metadata:
        if isinstance(metadata[key], str):
            metadata[key] = re.sub(r'\.\w+(\s|$|\[)', ' ', metadata[key])
    ...
    format_vars = {
        ...
        'episode': metadata['episode'] or '',
        'quality': metadata['quality'] or '',
        'audio': metadata['audio'] or '',
        'ext': ext.replace('.', ''),
        'Sseason': f"S{metadata['season']}" if metadata['season'] else '',
        'Eepisode': f"E{metadata['episode']}" if metadata['episode'] else '',
    }
    
    try:
        filename = template.format(**format_vars)
    except KeyError as e:
        logger.error(f"Invalid template variable: {e}")
        return None
    ...
    # Force .mkv extension (or whatever default you want)
...
def build_standardized_filename(
    ...
    """Build a standardized filename with consistent formatting."""
    parts = [clean_title(title)]
...
async def standardize_filename(filename: str, user_id: Optional[int] = None, caption: Optional[str] = None) -> str:
    ...
    try:
        ...
        # Use caption if specified and metadata source is set to caption
        source_text = caption if (user_id and caption and await DARKXSIDE78.get_metadata_source(user_id) == "caption") else base_name
        
        # Extract metadata
        season, episode = extract_season_episode(source_text)
        quality = extract_quality(source_text)
        audio = extract_language(source_text)
        ...
        logger.error(f"Error standardizing filename {filename}: {e}")
        return filename
def format_filename(metadata):
    """Format the filename according to the desired output patterns"""
    parts = []
    
    # Add season-episode in brackets if available
    ...
    # Add quality in brackets if available
    if metadata['quality']:
        parts.append(f"[{metadata['quality']}]")
    
    # Add audio/language in brackets if available
    if metadata['audio']:
        parts.append(f"[{metadata['audio']}]")
    
    # Join parts with spaces and clean up
    filename = ' '.join(parts).strip()
    filename = re.sub(r'\s+', ' ', filename)  # Remove extra spaces
...
async def auto_rename_files(client, message: Message):
    ...
    async def process_file():
        ...
        try:
            ...
            new_filename = await standardize_filename(
                file_name,
                user_id,
